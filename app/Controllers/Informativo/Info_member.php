...
class Info_member extends BaseController
{
    public $db;
    ...
    public function ajaxLoadData()
    {
        ...
        $like = array(
            'id_log_member' => $columns[0]['search']['value'],
            'id_member' => $columns[1]['search']['value'],
            'hour_in' => $columns[2]['search']['value'],
            'date' => $columns[3]['search']['value'],
            'status_log' => $columns[4]['search']['value']
        );


        $data = $logMemberModel->where('company', $_SESSION['company'])->findAll();
        $total_count = $data;
       
        $lib = new Datatable($logMemberModel, 'gp1', [ 'id_log_member ','id_member','hour_in','date','status_log','desc_status_log','year_act','company','created_at','updated_at','deleted_at']);
        $json_data = $lib->getResponse_Log_staff([
            'draw' => $_REQUEST['draw'],
